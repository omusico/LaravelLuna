...
//                $wheresql = ' where 1=1 ';
                ...
                if (env('SITE_TYPE', '') == 'five') {
                    ...
                    $lu_lotteries_bettings = \DB::select('select recUser.id,recUser.name,countTable.bcount,countTable.eachPrice,countTable.bingoPrice, countTable.profit from (' . $RecUsers
                        . ') recUser left join ( select uid,userName,sum(bcount) as bcount,sum(eachPrice) as eachPrice,sum(bingoPrice) as bingoPrice,sum(profit) as profit from ' .
